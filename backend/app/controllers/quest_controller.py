...
class QuestController:
    ...
    def complete_quest(quest_id, user_id, verification_data=None):
        ...
        try:
            ...
            if user:
                user.xp_balance = (user.xp_balance or 0) + quest.xp_reward
                user.total_xp_earned = (user.total_xp_earned or 0) + quest.xp_reward
                # NEW: Update quest completion count for user
            ...
            current_app.logger.info(f"[COMPLETE_QUEST] User {user_id} completed quest {quest_id}, earned {quest.xp_reward} XP")
            
            return {
                "message": f"Quest completed successfully! You earned {quest.xp_reward} XP!",
                "xp_awarded": quest.xp_reward,
    ...
    def verify_quest_completion(completion_id, admin_user_id, verification_decision, admin_notes=None):
        ...
        try:
            ...
            if verification_decision == 'VERIFIED':
                ...
                if user and quest:
                    completion.xp_awarded = quest.xp_reward
                    ...
                    points_log = PointsLog(
                        ...
                        points_awarded=quest.xp_reward
                    )
