...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        logger.info(f"[TOKEN_DECORATOR] {request.method} {request.url} - Authentication check")
        logger.info(f"[TOKEN_DECORATOR] Authorization header present: {bool(request.headers.get('Authorization'))}")
        logger.info(f"[TOKEN_DECORATOR] Request IP: {request.remote_addr}")
        logger.info(f"[TOKEN_DECORATOR] User Agent: {request.headers.get('User-Agent', 'Unknown')}")
        ...
        if auth_header:
            logger.debug(f"[TOKEN_DECORATOR] Authorization header: {auth_header[:20]}...")
            parts = auth_header.split()
            if len(parts) == 2 and parts[0].lower() == 'bearer':
                token = parts[1]
                logger.info(f"[TOKEN_DECORATOR] Token extracted successfully (length: {len(token)})")
                logger.debug(f"[TOKEN_DECORATOR] Token preview: {token[:20]}...")
            else:
                logger.warning(f"[TOKEN_DECORATOR] Invalid Authorization header format: {len(parts)} parts, first part: {parts[0] if parts else 'None'}")
        else:
            logger.warning(f"[TOKEN_DECORATOR] No Authorization header found")
            logger.info(f"[TOKEN_DECORATOR] Available headers: {list(request.headers.keys())}")

        if not token:
            logger.error(f"[TOKEN_DECORATOR] AUTHENTICATION FAILED - Token is missing from headers for {request.method} {request.url}")
            return jsonify({'message': 'Token is missing!'}), 401

        try:
            logger.info(f"[TOKEN_DECORATOR] Attempting to decode token with SECRET_KEY")
            secret_key = current_app.config.get('SECRET_KEY')
            logger.debug(f"[TOKEN_DECORATOR] SECRET_KEY available: {bool(secret_key)}")
            
            # Check token format before attempting to decode
            token_parts = token.split('.')
            if len(token_parts) != 3:
                logger.error(f"[TOKEN_DECORATOR] Invalid JWT format - expected 3 parts, got {len(token_parts)}. Token: {token[:50]}...")
                return jsonify({'message': 'Invalid token format - malformed JWT!'}), 401
            
            data = jwt.decode(token, secret_key, algorithms=['HS256'])
            logger.info(f"[TOKEN_DECORATOR] Token decoded successfully: {list(data.keys())}")
            logger.debug(f"[TOKEN_DECORATOR] Token payload: {data}")
            
            if 'admin_id' in data:
                admin_id = data['admin_id']
                logger.info(f"[TOKEN_DECORATOR] Admin token detected - admin_id: {admin_id}")
                ...
                if admin:
                    ...
                    logger.info(f"[TOKEN_DECORATOR] Admin authentication successful: {admin.username} (super_admin)")
                    logger.debug(f"[TOKEN_DECORATOR] Admin details - ID: {admin.id}, Username: {admin.username}, Email: {admin.email}")
                else:
                    logger.error(f"[TOKEN_DECORATOR] AUTHENTICATION FAILED - Admin with id {admin_id} from token not found in DB.")
                    logger.info(f"[TOKEN_DECORATOR] Available admins in DB: {[a.id for a in Admin.query.all()]}")
                    return jsonify({'message': 'Invalid admin token - user not found!'}), 401
                    
            elif 'user_id' in data:
                user_id = data['user_id']
                logger.info(f"[TOKEN_DECORATOR] User token detected - user_id: {user_id}")
                ...
                if user:
                    ...
                    logger.info(f"[TOKEN_DECORATOR] User authentication successful: {user.username} ({user.role})")
                    logger.debug(f"[TOKEN_DECORATOR] User details - ID: {user.id}, Username: {user.username}, Email: {user.email}, Role: {user.role}")
                    if hasattr(user, 'business_id') and user.business_id:
                        logger.debug(f"[TOKEN_DECORATOR] User business_id: {user.business_id}")
                else:
                    logger.error(f"[TOKEN_DECORATOR] AUTHENTICATION FAILED - User with id {user_id} from token not found in DB.")
                    logger.info(f"[TOKEN_DECORATOR] Attempting to query User table...")
                    try:
                        available_users = User.query.all()
                        logger.info(f"[TOKEN_DECORATOR] Available users in DB: {[u.id for u in available_users]}")
                    except Exception as e:
                        logger.error(f"[TOKEN_DECORATOR] Error querying users: {e}")
                    return jsonify({'message': 'Invalid user token - user not found!'}), 401
            else:
                logger.error(f"[TOKEN_DECORATOR] FATAL: Token payload MISSING both 'admin_id' and 'user_id': {data}")
                logger.error(f"[TOKEN_DECORATOR] Available keys in token: {list(data.keys())}")
                return jsonify({'message': 'Invalid token payload - missing user identification!'}), 401
                
            logger.info(f"[TOKEN_DECORATOR] ✅ Authentication completed successfully for {g.current_user.username} ({g.user_role})")
            logger.info(f"[TOKEN_DECORATOR] Proceeding to execute {f.__name__} function")
            
            result = f(*args, **kwargs)
            logger.info(f"[TOKEN_DECORATOR] Function {f.__name__} completed successfully")
            return result
            
        except jwt.ExpiredSignatureError:
            logger.warning(f"[TOKEN_DECORATOR] AUTHENTICATION FAILED - Token has expired for {request.method} {request.url}")
            logger.debug(f"[TOKEN_DECORATOR] Expired token preview: {token[:20]}...")
            return jsonify({'message': 'Token has expired!'}), 401
        except jwt.InvalidTokenError as e:
            logger.error(f"[TOKEN_DECORATOR] AUTHENTICATION FAILED - Invalid token for {request.method} {request.url}: {str(e)}")
            logger.debug(f"[TOKEN_DECORATOR] Invalid token preview: {token[:20]}...")
            return jsonify({'message': f'Token is invalid: {str(e)}'}), 401
        except Exception as e:
            logger.error(f"[TOKEN_DECORATOR] AUTHENTICATION FAILED - Unexpected error for {request.method} {request.url}: {str(e)}", exc_info=True)
            logger.debug(f"[TOKEN_DECORATOR] Problem token preview: {token[:20] if token else 'None'}...")
...
def token_optional(f):
    ...
    def decorated_optional(*args, **kwargs):
        logger.info(f"[TOKEN_OPTIONAL_DECORATOR] {request.method} {request.url} - Optional authentication check")
        token = None
        auth_header = request.headers.get('Authorization')
        g.current_user = None
        g.current_admin = None
        ...
        if auth_header:
            ...
            if len(parts) == 2 and parts[0].lower() == 'bearer':
                token = parts[1]
                logger.info(f"[TOKEN_OPTIONAL_DECORATOR] Token extracted (length: {len(token)})")
            else:
                logger.warning(f"[TOKEN_OPTIONAL_DECORATOR] Invalid Authorization header format.")
        else:
            logger.info(f"[TOKEN_OPTIONAL_DECORATOR] No Authorization header found, proceeding as anonymous.")

        if token:
            try:
                secret_key = current_app.config.get('SECRET_KEY')
                data = jwt.decode(token, secret_key, algorithms=['HS256'])
                logger.info(f"[TOKEN_OPTIONAL_DECORATOR] Token decoded successfully: {list(data.keys())}")
                ...
                if 'admin_id' in data:
                    ...
                    if admin:
                        ...
                        g.current_admin = admin
                        g.user_role = 'super_admin'
                        logger.info(f"[TOKEN_OPTIONAL_DECORATOR] Admin authentication successful: {admin.username} (super_admin)")
                    else:
                        logger.warning(f"[TOKEN_OPTIONAL_DECORATOR] Admin ID {data['admin_id']} from token not found.")
                elif 'user_id' in data:
                    user = User.query.get(data['user_id'])
                    if user:
                        g.current_user = user
                        g.user_role = user.role
                        logger.info(f"[TOKEN_OPTIONAL_DECORATOR] User authentication successful: {user.username} ({user.role})")
                    else:
                        logger.warning(f"[TOKEN_OPTIONAL_DECORATOR] User ID {data['user_id']} from token not found.")
                else:
                    logger.warning(f"[TOKEN_OPTIONAL_DECORATOR] Token payload missing 'admin_id' and 'user_id'.")

            except jwt.ExpiredSignatureError:
                logger.warning(f"[TOKEN_OPTIONAL_DECORATOR] Token has expired. Proceeding as anonymous.")
            except jwt.InvalidTokenError as e:
                logger.warning(f"[TOKEN_OPTIONAL_DECORATOR] Invalid token: {str(e)}. Proceeding as anonymous.")
            except Exception as e:
                logger.error(f"[TOKEN_OPTIONAL_DECORATOR] Unexpected error during token decoding: {str(e)}", exc_info=True)
        
        logger.info(f"[TOKEN_OPTIONAL_DECORATOR] Proceeding to execute {f.__name__}. User: {g.current_user.username if g.current_user else 'Anonymous'}, Role: {g.user_role}")
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        logger.info(f"[ADMIN_REQUIRED] Path: {request.path}")
        if not hasattr(g, 'user_role'):
            logger.warning("[ADMIN_REQUIRED] 'g.user_role' not set. Denying access.")
            return jsonify(message="Authentication context error."), 403
        
        logger.info(f"[ADMIN_REQUIRED] Checking role: g.user_role = '{g.user_role}'")
        if g.user_role != 'super_admin':
            logger.warning(f"[ADMIN_REQUIRED] Access denied. Role '{g.user_role}' is not 'super_admin'.")
            return jsonify(message="Super admin access required for this resource."), 403
        
        logger.info(f"[ADMIN_REQUIRED] Access GRANTED for role '{g.user_role}'.")
        logger.info(f"--- [ADMIN_REQUIRED END] ---")
        return f(*args, **kwargs)
...
def business_admin_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        logger.info(f"--- [BUSINESS_ADMIN_REQUIRED CHECK] ---")
        logger.info(f"[BUSINESS_ADMIN_REQUIRED] Path: {request.path}")
        
        if not hasattr(g, 'user_role'):
            logger.warning("[BUSINESS_ADMIN_REQUIRED] 'g.user_role' not set.")
            return jsonify(message="Authentication context error."), 403
        
        logger.info(f"[BUSINESS_ADMIN_REQUIRED] Checking role: g.user_role = '{g.user_role}'")
        
        # Super admins bypass business admin requirements
        if g.user_role == 'super_admin':
            logger.info(f"[BUSINESS_ADMIN_REQUIRED] ✅ Super admin {g.current_user.username} bypassing business admin check")
            return f(*args, **kwargs)
        
        if g.user_role != 'business_admin':
            logger.warning(f"[BUSINESS_ADMIN_REQUIRED] Access denied. Role '{g.user_role}' is not 'business_admin' or 'super_admin'.")
        ...
        if not hasattr(g, 'current_user') or not g.current_user.business_id:
            logger.warning(f"[BUSINESS_ADMIN_REQUIRED] Business admin user ID {g.current_user.id if hasattr(g, 'current_user') else 'N/A'} is not associated with a business_id.")
            return jsonify(message="Business admin is not associated with a business."), 403
        
        logger.info(f"[BUSINESS_ADMIN_REQUIRED] Access GRANTED for Business Admin (ID: {g.current_user.id}) for Business ID: {g.current_user.business_id}.")
        logger.info(f"--- [BUSINESS_ADMIN_REQUIRED END] ---")
        return f(*args, **kwargs)
    return decorated_function
...
def business_admin_scoped_permission_required(required_permission=None):
    def decorator(f):
        @wraps(f)
        def decorated_function(*args, **kwargs):
            logger.info(f"--- [BUSINESS_SCOPED_PERM CHECK for {f.__name__}] ---")
            if not hasattr(g, 'current_user') or not g.current_user or not hasattr(g, 'user_role'):
                logger.error("[BUSINESS_SCOPED_PERM] Auth context not found.")
            ...
            if target_business_id_from_route is None:
                ...
                if target_business_id_from_route is None:
                    logger.error(f"[BUSINESS_SCOPED_PERM] Cannot determine target_business_id for user {user.id}.")
            ...
            try:
                ...
                logger.error(f"[BUSINESS_SCOPED_PERM] Invalid business_id format: {target_business_id_from_route}")
                return jsonify(message="Invalid business ID format."), 400

            if role == 'super_admin':
                logger.info(f"[BUSINESS_SCOPED_PERM] Super Admin access GRANTED for Business ID {target_business_id}.")
                g.target_business = Business.query.get(target_business_id)
                return f(*args, **kwargs)

            if role != 'business_admin':
                logger.warning(f"[BUSINESS_SCOPED_PERM] Access DENIED. User {user.id} Role '{role}' not 'business_admin' or 'super_admin'.")
            ...
            if assigned_business_id != target_business_id:
                logger.warning(f"[BUSINESS_SCOPED_PERM] Access DENIED. BA {user.id} for Business {assigned_business_id}, target {target_business_id}.")
            ...
            if required_permission:
                ...
                if not business_obj or not business_obj.permissions or not business_obj.permissions.get(required_permission, False):
                    logger.warning(f"[BUSINESS_SCOPED_PERM] Access DENIED for BA {user.id}. Business ID {target_business_id} lacks permission '{required_permission}'.")
                ...
                if not admin_permissions.get(required_permission, False):
                    logger.warning(f"[BUSINESS_SCOPED_PERM] Access DENIED for BA {user.id}. Missing personal permission: '{required_permission}'.")
                    return jsonify(message=f"Access denied. You do not have the required permission: '{required_permission}'."), 403
                
                logger.info(f"[BUSINESS_SCOPED_PERM] BA {user.id} HAS permission: '{required_permission}'.")

            logger.info(f"[BUSINESS_SCOPED_PERM] Access GRANTED for BA {user.id} to Business ID {target_business_id}.")
            return f(*args, **kwargs)
        return decorated_function
    return decorator
...
def enforce_business_access(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not business_id:
            logger.error("[ENFORCE_BUSINESS_ACCESS] Business ID not found in route kwargs.")
            return jsonify(message="Business ID missing from request."), 400
        
        business = Business.query.get(business_id)
        if not business:
            logger.warning(f"[ENFORCE_BUSINESS_ACCESS] Business not found: {business_id}")
            return jsonify(message="Business not found."), 404

        g.target_business = business
        logger.info(f"[ENFORCE_BUSINESS_ACCESS] Checking access for Business ID: {business_id}, Name: '{business.name}', Audience Type: {business.audience_type}")
        logger.info(f"[ENFORCE_BUSINESS_ACCESS] User role: {g.get('user_role')}, User ID: {g.current_user.id if hasattr(g, 'current_user') and g.current_user else 'Anonymous'}")

        if hasattr(g, 'user_role') and g.user_role == 'super_admin':
            logger.info(f"[ENFORCE_BUSINESS_ACCESS] Super Admin access GRANTED for Business {business_id}.")
            return f(*args, **kwargs)
        ...
        if hasattr(g, 'user_role') and g.user_role == 'business_admin':
            if hasattr(g, 'current_user') and g.current_user and g.current_user.business_id == business.id:
                logger.info(f"[ENFORCE_BUSINESS_ACCESS] Business Admin {g.current_user.id} access GRANTED for their assigned Business {business_id}.")
                return f(*args, **kwargs)
            else:
                logger.warning(f"[ENFORCE_BUSINESS_ACCESS] Business Admin {g.current_user.id if hasattr(g, 'current_user') and g.current_user else 'N/A'} access DENIED. Target Business {business_id} is not their assigned business (assigned: {g.current_user.business_id if hasattr(g, 'current_user') and g.current_user else 'N/A'}).")
                return jsonify(message="Access denied. You can only access your assigned business."), 403

        if business.audience_type == 'PUBLIC':
            logger.info(f"[ENFORCE_BUSINESS_ACCESS] Access GRANTED for PUBLIC Business {business_id} (User role: {g.get('user_role', 'Anonymous')}).")
            return f(*args, **kwargs)
        ...
        if business.audience_type == 'RESTRICTED':
            if hasattr(g, 'current_user') and g.current_user and hasattr(g, 'user_role') and g.user_role == 'user':
                if BusinessController.check_user_access(g.current_user, business.id):
                    logger.info(f"[ENFORCE_BUSINESS_ACCESS] Authenticated 'user' {g.current_user.id} access GRANTED to RESTRICTED Business {business_id} via check_user_access.")
                    return f(*args, **kwargs)
                else:
                    logger.warning(f"[ENFORCE_BUSINESS_ACCESS] Authenticated 'user' {g.current_user.id} access DENIED to RESTRICTED Business {business_id} via check_user_access.")
                    return jsonify(message="You do not have permission to access this business based on its audience rules."), 403
            else:
                logger.warning(f"[ENFORCE_BUSINESS_ACCESS] Access DENIED for RESTRICTED Business {business_id}. Role: {g.get('user_role', 'Anonymous')}. Authentication as a 'user' role and specific audience criteria required.")
                return jsonify(message="Authentication required and specific audience criteria must be met to access this business."), 401

        logger.error(f"[ENFORCE_BUSINESS_ACCESS] Access DENIED - Fallback. Business: {business_id}, Role: {g.get('user_role', 'Anonymous')}")
    ...
    def decorated(*args, **kwargs):
        try:
            ...
            if user_role == 'super_admin':
                logger.info(f"[SURVEY_ACCESS] ✅ Super admin bypass granted for survey {survey_id}")
                ...
                if current_user and current_user.business_id == survey.business_id:
                    logger.info(f"[SURVEY_ACCESS] ✅ Business admin bypass granted for own business survey {survey_id}")
                    g.direct_access = False
                    return f(*args, **kwargs)
                else:
                    logger.warning(f"[SURVEY_ACCESS] ❌ Business admin {current_user.id} cannot access survey from different business")
            ...
            if not has_access:
                logger.warning(f"Access to survey {survey_id} denied for user {current_user.id}. Reason: {reason}")
...
def check_user_access(user, audience_settings):
    if not audience_settings:
        logger.debug("[AUTH_CHECK_USER_ACCESS_SURVEY] No specific audience settings for survey, access granted by this check.")
        return True
    ...
    if audience_settings.specific_email_whitelist and user.email.lower() in [e.lower() for e in audience_settings.specific_email_whitelist]:
        logger.debug(f"[AUTH_CHECK_USER_ACCESS_SURVEY] User {user.email} GRANTED via specific email whitelist.")
        return True
    ...
    if audience_settings.email_domain_whitelist:
        ...
        if user_domain and user_domain in [d.lower() for d in audience_settings.email_domain_whitelist]:
            logger.debug(f"[AUTH_CHECK_USER_ACCESS_SURVEY] User {user.email} GRANTED via email domain whitelist ({user_domain}).")
            return True
            
    if user.discord_id and audience_settings.discord_roles_allowed:
        logger.debug(f"[AUTH_CHECK_USER_ACCESS_SURVEY] Performing Discord role check for user {user.discord_id}.")
        # Find linked account
            ...
            if survey and survey.business and survey.business.discord_server:
                ...
                if "error" not in member_info:
                    ...
                    if user_roles.intersection(required_role_set):
                        logger.debug(f"[AUTH_CHECK_USER_ACCESS_SURVEY] User {user.email} GRANTED via Discord role.")
                        return True
                    
        logger.debug(f"[AUTH_CHECK_USER_ACCESS_SURVEY] Discord role check failed for user {user.discord_id}.")
        
    logger.debug(f"[AUTH_CHECK_USER_ACCESS_SURVEY] User {user.email} DENIED by survey-specific rules.")
    return False
...
def check_business_ai_points(points_required_func):
    ...
    def decorator(f):
        @wraps(f)
        def decorated_function(*args, **kwargs):
            logger.info(f"[AI_POINTS_CHECK] === STARTING AI POINTS CHECK ===")
            logger.info(f"[AI_POINTS_CHECK] Function: {f.__name__}")
            logger.info(f"[AI_POINTS_CHECK] URL: {request.method} {request.url}")
            logger.info(f"[AI_POINTS_CHECK] Remote IP: {request.remote_addr}")
            logger.info(f"[AI_POINTS_CHECK] Headers: Authorization={bool(request.headers.get('Authorization'))}")
            
            # Log g object state
            logger.info(f"[AI_POINTS_CHECK] g object attributes: {[attr for attr in dir(g) if not attr.startswith('_')]}")
            logger.info(f"[AI_POINTS_CHECK] g.user_role exists: {hasattr(g, 'user_role')}")
            logger.info(f"[AI_POINTS_CHECK] g.current_user exists: {hasattr(g, 'current_user')}")
            
            if hasattr(g, 'user_role'):
                logger.info(f"[AI_POINTS_CHECK] g.user_role value: '{g.user_role}' (type: {type(g.user_role)})")
            else:
                logger.error(f"[AI_POINTS_CHECK] CRITICAL: g.user_role not found in g object!")
                return jsonify({"error": "Authentication context missing - user role not set"}), 500
                
            if hasattr(g, 'current_user'):
                if g.current_user:
                    logger.info(f"[AI_POINTS_CHECK] g.current_user: ID={getattr(g.current_user, 'id', 'N/A')}, Username={getattr(g.current_user, 'username', 'N/A')}")
                    logger.info(f"[AI_POINTS_CHECK] g.current_user type: {type(g.current_user)}")
                    if hasattr(g.current_user, 'role'):
                        logger.info(f"[AI_POINTS_CHECK] g.current_user.role: '{g.current_user.role}'")
                    if hasattr(g.current_user, 'business_id'):
                        logger.info(f"[AI_POINTS_CHECK] g.current_user.business_id: {g.current_user.business_id}")
                else:
                    logger.error(f"[AI_POINTS_CHECK] CRITICAL: g.current_user is None!")
            else:
                logger.error(f"[AI_POINTS_CHECK] CRITICAL: g.current_user not found in g object!")
                
            # Super admins bypass ALL AI points and permission checks
            if g.user_role == 'super_admin':
                logger.info(f"[AI_POINTS_CHECK] ✅ SUPER ADMIN BYPASS CONFIRMED")
                logger.info(f"[AI_POINTS_CHECK] User {g.current_user.username if g.current_user else 'UNKNOWN'} bypassing ALL AI restrictions")
                # Set default values for compatibility but allow unlimited access
                g.business = None
                g.points_needed = 0
                logger.info(f"[AI_POINTS_CHECK] Set g.business=None, g.points_needed=0 for super admin")
                logger.info(f"[AI_POINTS_CHECK] === SUPER ADMIN BYPASS COMPLETE - PROCEEDING TO FUNCTION ===")
                return f(*args, **kwargs)

            logger.info(f"[AI_POINTS_CHECK] Not a super admin, proceeding with business admin checks...")

            # Only business admins are subject to AI points checking
            if g.user_role != 'business_admin':
                logger.warning(f"[AI_POINTS_CHECK] ❌ ACCESS DENIED: User {g.current_user.username if g.current_user else 'UNKNOWN'} with role '{g.user_role}' trying to access AI feature")
                logger.warning(f"[AI_POINTS_CHECK] Only 'super_admin' and 'business_admin' roles are allowed")
                return jsonify({"error": "This action requires a business account"}), 403

            logger.info(f"[AI_POINTS_CHECK] User role 'business_admin' confirmed, checking business association...")

            business_id = getattr(g.current_user, 'business_id', None)
            if not business_id:
                logger.error(f"[AI_POINTS_CHECK] ❌ ACCESS DENIED: Business admin {g.current_user.username} has no business_id")
                return jsonify({"error": "User is not associated with a business"}), 403

            logger.info(f"[AI_POINTS_CHECK] Business ID found: {business_id}, fetching business details...")

            business = Business.query.get(business_id)
            if not business:
                logger.error(f"[AI_POINTS_CHECK] ❌ ACCESS DENIED: Business {business_id} not found for user {g.current_user.username}")
                return jsonify({"error": "Business not found"}), 404

            logger.info(f"[AI_POINTS_CHECK] Business found: {business.name} (ID: {business.id})")

            # Calculate required points based on request data
            try:
                points_needed = points_required_func(request)
                logger.info(f"[AI_POINTS_CHECK] Points calculation: needed={points_needed}, available={business.ai_points}")
            except Exception as e:
                logger.error(f"[AI_POINTS_CHECK] ❌ ERROR calculating required points: {e}")
                return jsonify({"error": "Error calculating required points"}), 500

            # Check AI points permission
            permissions = business.permissions or {}
            logger.info(f"[AI_POINTS_CHECK] Business permissions: {permissions}")
            ...
            if not (has_ai_builder_perm or can_create_surveys):
                logger.warning(f"[AI_POINTS_CHECK] ❌ PERMISSION DENIED: Business {business.name} lacks AI permission and cannot create surveys")
                return jsonify({
            ...
            if business.ai_points < points_needed:
                logger.warning(f"[AI_POINTS_CHECK] ❌ INSUFFICIENT POINTS: Business {business.name} - needed: {points_needed}, available: {business.ai_points}")
                return jsonify({
            ...
            logger.info(f"[AI_POINTS_CHECK] ✅ AI points check passed for business {business.name}")
            logger.info(f"[AI_POINTS_CHECK] === BUSINESS ADMIN CHECK COMPLETE - PROCEEDING TO FUNCTION ===")
            return f(*args, **kwargs)
        return decorated_function
    return decorator
...
def check_business_permission(permission_key):
    ...
    def decorator(f):
        @wraps(f)
        def decorated_function(*args, **kwargs):
            logger.info(f"[PERMISSION_CHECK] Checking permission '{permission_key}' for {request.method} {request.url}")
            
            # Super admins bypass permission checks
            if g.user_role == 'super_admin':
                logger.info(f"[PERMISSION_CHECK] Super admin {g.current_user.username} bypassing permission check")
                return f(*args, **kwargs)

            # Only business admins are subject to permission checking
            if g.user_role != 'business_admin':
                logger.warning(f"[PERMISSION_CHECK] User {g.current_user.username} with role {g.user_role} trying to access business feature")
                return jsonify({"error": "This action requires a business account"}), 403

            business_id = getattr(g.current_user, 'business_id', None)
            if not business_id:
                logger.error(f"[PERMISSION_CHECK] Business admin {g.current_user.username} has no business_id")
                return jsonify({"error": "User is not associated with a business"}), 403

            business = Business.query.get(business_id)
            if not business:
                logger.error(f"[PERMISSION_CHECK] Business {business_id} not found for user {g.current_user.username}")
                return jsonify({"error": "Business not found"}), 404

            # Check permission
            permissions = business.permissions or {}
            if not permissions.get(permission_key, False):
                logger.warning(f"[PERMISSION_CHECK] Business {business.name} does not have permission '{permission_key}'")
                return jsonify({
            ...
            # Store business object for use in the route
            g.business = business
            logger.info(f"[PERMISSION_CHECK] ✅ Permission check passed for business {business.name}")
            return f(*args, **kwargs)
        return decorated_function
    return decorator
