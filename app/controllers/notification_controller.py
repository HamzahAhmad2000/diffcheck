...
try:
    from app.websocket_manager import emit_notification, emit_admin_notification, emit_broadcast_notification
    WEBSOCKET_AVAILABLE = True
except ImportError:
    logger.warning("WebSocket manager not available. Real-time notifications disabled.")
    ...
    try:
        ...
        if WEBSOCKET_AVAILABLE:
            try:
                ...
                emit_notification(target_user_id, notification_data)
                logger.info(f"Real-time notification emitted to user {target_user_id}")
            except Exception as ws_error:
                logger.error(f"Failed to emit WebSocket notification: {ws_error}")
    ...
    try:
        ...
        if WEBSOCKET_AVAILABLE:
            for notification in notifications:
                try:
                    notification_data = notification.to_dict()
                    emit_notification(notification.user_id, notification_data)
                except Exception as ws_error:
                    logger.error(f"Failed to emit WebSocket notification for user {notification.user_id}: {ws_error}")
            logger.info(f"Real-time notifications emitted to {len(notifications)} users")
