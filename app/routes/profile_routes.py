...
from ..controllers.profile_controller import (
    ...
    get_linked_accounts, mark_welcome_popup_seen
)
...
@profile_bp.route('/profile/welcome-popup-seen', methods=['POST'])
@token_required
def mark_welcome_popup_seen_route():
    ...
    try:
        ...
        user_id = g.current_user.id
        result, status_code = mark_welcome_popup_seen(user_id)
        return jsonify(result), status_code
