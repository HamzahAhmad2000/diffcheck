...
class User(db.Model):
    ...
    # NEW: Share to Earn XP tracking
    has_completed_join_share = db.Column(db.Boolean, default=False, nullable=False)  # Track if user completed first-time join share

    # NEW: Referral tracking
    pending_referral_code = db.Column(db.String(32), nullable=True)  # Store referral code during registration process

    # NEW: Welcome popup tracking
    has_seen_welcome_popup = db.Column(db.Boolean, default=False, nullable=False)  # Track if user has seen the welcome popup

    # NEW: Legal compliance tracking
    has_accepted_legal_terms = db.Column(db.Boolean, default=False, nullable=False)  # Track if user has accepted legal terms
    legal_terms_accepted_at = db.Column(db.DateTime, nullable=True)  # Timestamp when legal terms were accepted
    has_seen_ai_policy = db.Column(db.Boolean, default=False, nullable=False)  # Track if user has seen AI policy
    ai_policy_accepted_at = db.Column(db.DateTime, nullable=True)  # Timestamp when AI policy was accepted
    ...
    def to_dict(self):
        data = {
            ...
            "has_seen_welcome_popup": self.has_seen_welcome_popup,  # Include welcome popup tracking
            "has_completed_join_share": self.has_completed_join_share,  # Include share-to-earn tracking
            "has_accepted_legal_terms": self.has_accepted_legal_terms,  # Include legal terms acceptance
            "legal_terms_accepted_at": self.legal_terms_accepted_at.isoformat() if self.legal_terms_accepted_at else None,  # Include legal terms acceptance timestamp
            "has_seen_ai_policy": self.has_seen_ai_policy,  # Include AI policy acceptance
            "ai_policy_accepted_at": self.ai_policy_accepted_at.isoformat() if self.ai_policy_accepted_at else None,  # Include AI policy acceptance timestamp
        }
...
# NEW: Share to Earn XP System Models

class ShareType(Enum):
    """Types of sharing actions that can earn XP"""
...
class UserShare(db.Model):
    """Track user sharing actions to prevent duplicate XP rewards"""
    __tablename__ = 'user_shares'
    
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id', ondelete='CASCADE'), nullable=False)
    share_type = db.Column(db.String(50), nullable=False)  # From ShareType enum
    related_object_id = db.Column(db.Integer, nullable=True)  # Badge ID, Reward ID, Raffle ID, etc.
    ...
    # Unique constraint to prevent duplicate shares for the same object
    __table_args__ = (
        db.UniqueConstraint('user_id', 'share_type', 'related_object_id', name='uq_user_share_object'),
        db.Index('idx_user_share_type', 'user_id', 'share_type'),
    )
...
class SystemConfiguration(db.Model):
    """System-wide configuration settings for features like Share to Earn XP"""
    __tablename__ = 'system_configurations'
    
    id = db.Column(db.Integer, primary_key=True)
    config_key = db.Column(db.String(100), nullable=False, unique=True)
    config_value = db.Column(db.Text, nullable=False)
    config_type = db.Column(db.String(50), nullable=False, default='string')  # 'string', 'integer', 'boolean', 'json'
    description = db.Column(db.Text, nullable=True)
    category = db.Column(db.String(50), nullable=False, default='general')  # 'share_to_earn', 'general', etc.
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    def get_typed_value(self):
        """Return the config value converted to its proper type"""
    ...
    def set_typed_value(self, value):
        """Set the config value with proper type conversion"""
    ...
    def get_config(key, default=None):
        ...
        return config.get_typed_value() if config else default
    
    @staticmethod
    def set_config(key, value, config_type='string', description=None, category='general'):
...
class ShareAnalytics(db.Model):
    """Track analytics for share-to-earn feature"""
    __tablename__ = 'share_analytics'
    
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id', ondelete='CASCADE'), nullable=True)
    event_type = db.Column(db.String(50), nullable=False)  # 'prompt_shown', 'button_clicked', 'share_completed'
    share_type = db.Column(db.String(50), nullable=False)  # From ShareType enum
    related_object_id = db.Column(db.Integer, nullable=True)
    session_id = db.Column(db.String(100), nullable=True)  # To track user sessions
    user_agent = db.Column(db.String(255), nullable=True)
    ip_address = db.Column(db.String(45), nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    
    # Relationships
    user = db.relationship('User', backref='share_analytics')
    
    # Indexes for analytics queries
    __table_args__ = (
        db.Index('idx_share_analytics_event', 'event_type', 'share_type', 'created_at'),
        db.Index('idx_share_analytics_user', 'user_id', 'created_at'),
    )
...
class CryptoPayment(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
    charge_id = db.Column(db.String(128), unique=True, nullable=False)
    hosted_url = db.Column(db.String(512))
    amount = db.Column(db.Float)
    currency = db.Column(db.String(10), default="USD")
    status = db.Column(db.String(50), default="PENDING")
    product_type = db.Column(db.String(100))  # e.g. ai_points, response_package
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
